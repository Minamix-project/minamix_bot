...
async def modify_user_balance(db: pymysql.connections.Connection, guild_id: int, user_id: int, amount: int, operation: str = "add") -> int:
    with db.cursor() as cursor:
        ...
        if operation == "add": value = current + amount
        elif operation == "remove": value = max(current - amount, 0)
        elif operation == "set": value = amount
        else: raise ValueError(f"Opération inconnue : {operation}")
